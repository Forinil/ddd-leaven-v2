...
@Finder
class JpaShipmentFinder : ShipmentFinder {

    @PersistenceContext
    private val entityManager: EntityManager? = null

    override fun findShipment(): List<ShipmentDto> {
        val jpql = "select new pl.com.bottega.erp.shipping.presentation.ShipmentDto(s.id, s.orderId, s.status) from Shipment s"
        val query = entityManager!!.createQuery(jpql)
        return query.resultList.toList() as List<ShipmentDto>
    }
}
