...
class ShipmentsListController {
    ...
    @Inject
    private val finder: ShipmentFinder? = null

    @Inject
    private val gate: Gate? = null

    @RequestMapping("/list")
    fun shippingList(model: Model): String {
        val shipments = finder!!.findShipment()
        model.addAttribute("shipments", shipments)
        return "/shipping/shipmentsList"
    }

    @RequestMapping(value = "/send", method = arrayOf(RequestMethod.POST))
    fun shipOrder(@RequestParam("shipmentId") shipmentId: String): String {
        gate!!.dispatch(SendShipmentCommand(AggregateId(shipmentId)))
        return "redirect:/shipping/shipment/list"
    }

    @RequestMapping(value = "/deliver", method = arrayOf(RequestMethod.POST))
    fun receiveShipment(@RequestParam("shipmentId") shipmentId: String): String {
        gate!!.dispatch(DeliverShipmentCommand(AggregateId(shipmentId)))
        return "redirect:/shipping/shipment/list"
    }
}
