...
class DeliverShipmentCommandHandler : CommandHandler<DeliverShipmentCommand, Unit> {

    @Inject
    private val repository: ShipmentRepository? = null

    override fun handle(command: DeliverShipmentCommand) {
        val shipment = repository!!.load(command.shipmentId)
        shipment.deliver()
    }
}
