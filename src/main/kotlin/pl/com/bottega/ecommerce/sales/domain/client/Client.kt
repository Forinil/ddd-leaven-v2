...
@Entity
@AggregateRoot
class Client : BaseAggregateRoot() {

    private val name: String? = null

    @Inject
    @Transient
    private val paymentFactory: PaymentFactory? = null

    fun generateSnapshot(): ClientData {
        return ClientData(aggregateId, name!!)
    }
    ...
    /**
     * Sample model: one aggregate creates another.<br></br>
     * Client model does not compose Payment - therefore it does not manage Payment lifecycle.<br></br>
     * Application layer is responsible for managing Payment lifecycle;
     *
     * @param amount
     * @return
     */
    fun charge(amount: Money): Payment {
        if (!canAfford(amount)) {
